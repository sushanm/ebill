...
function AddBatch({ productId, newProduct, callBackMethod, saleMode, productName, callbackaftersalesFromBatch, usedFor, gst, giveDiscountEdit }) {
    ...
    const addToSale = (batch, quantity) => {
        let saleData = {
            id: productId,
            name: productName,
            gst: gst,
            ...
            }
            {
                ...
                <>
                    <div className="row">
                        ...
                        <div className="col-2 batch-custom-col-h"> Quantity</div>
                        <div className="col-4 batch-custom-col-h">Expiry Date</div>
                        <div className="col-4 batch-custom-col-h"> Action </div>
                    </div>
                    {
                        !editMode &&
                        <div className="row">
                            ...
                            <div className="col-2 batch-custom-col"> <input type={'number'} placeholder="Quantity" value={quantity} onChange={(evt) => SetQuantity(evt.target.value)}></input></div>
                            ...
                            <div className="col-4 batch-custom-col"> <button onClick={() => AddOrEditBatch(false, -1)} disabled={disableAdd} >Add</button></div>
                        </div>
                    }
                                ...
                                <div key={index} className={`row batch-custom-row  ${item.nearToExpiry== true ? 'blink' : ''}`} >
                                    {
                                        editBatchRow === index ? <>
                                            ...
                                            <div className="col-2 batch-custom-col"> <input type={'number'} value={quantity} placeholder="Quantity" onChange={(evt) => SetQuantity(evt.target.value)}></input></div>
                                            <div className="col-4 batch-custom-col"><input type={'month'} placeholder="Expiry Date" value={expiryDate} onChange={(evt) => SetExpiryDate(evt.target.value)}></input></div>

                                            <div className="col-4 batch-custom-col"> <button onClick={() => AddOrEditBatch(true, item.id)} >Save</button></div>
                                        </> : <>
                                            <div className="col-2 batch-custom-col">
                                                {item.price}
                                            </div>
                                            <div className="col-1 batch-custom-col">{item.quantity}</div>
                                            <div className="col-2 batch-custom-col">{item.expiryDate}</div>
