...
function Report() {
  ...
  const [transactioByDateTotalPrice, SetTransactionByDateTotalPrice] =
    useState();
  ...
  const getAllProducts = async (force) => {
    ...
    if (allProducts || force) {
      ...
      setProductsWithoutGST(
        allProducts.filter((item) => item.gst == null || item.gst == -1)
      );
    ...
    await TransactionsDataService.getAllTransactions().then((data) => {
      if (data) {
          ...
          element.transactions.forEach((trans) => {
            if (trans.saleDate) {
              ...
              if (trnsDate <= todayDate) {
                transAmountTillDate =
                  Number(transAmountTillDate) +
                  Number(trans.totalPrice) -
        ...
        localStorage.setItem(
          "drkotianTransactionDataByMonth",
          JSON.stringify(data)
        );
        ...
        if (data) {
          data.forEach((element) => {
            if (element.transactions) {
              element.transactions.forEach((trans) => {
                if (trans.mobile) {
                  mobileNumbers.push(trans.mobile.trim());
                }
  ...
  const filterItemsByMonthAndQuantity = (month, quantity) => {
    ...
    if (transactionsByItems) {
      ...
      var result = Object.keys(itemsByGroup).map((key) => [
        key,
        itemsByGroup[key],
      ]);
    ...
    var result2 = Object.keys(groupByItem).map((key) => [
      key,
      groupByItem[key],
    ]);
    ...
    result2.forEach((element) => {
      ...
      element[1].forEach((items) => {
        ...
        if (totalQunatity >= quantity) {
          ...
          if (
            itemsByMonthAndQantity.filter((val) => val.id === items.id)
              .length === 0
          ) {
            itemsByMonthAndQantity.push(items);
          } else {
            let index = itemsByMonthAndQantity.findIndex(
              (i) => i.id === items.id
            );
  ...
  useEffect(() => {
    ...
    if (seachProduct) {
      ...
      SetTransactionsByItemsDisplay(
        clone.filter((item) => search.test(item.name.toLowerCase()))
      );
    }
  }, [seachProduct]);
  ...
  const transactionsByDate = (date) => {
    if (transactioByMonth) {
      ...
      const dateFormat =
        ...
        "-" +
        date.split("-")[0];
      let tempData = transactioByMonth.filter(
        (item) => item.id === yearMonth
      )[0];
      if (tempData) {
        tempData = tempData.transactions.filter(
          (ele) => ele.saleDate === dateFormat
        );
        let tempTotalPrice = 0;
        tempData.forEach((element) => {
          tempTotalPrice =
            Number(tempTotalPrice) +
            (Number(element.totalPrice) - Number(element.discount));
        });
  ...
  const ShowAllTaxInfo = (value) => {
    ...
    if (!ShowAllTax) {
      ...
      setProductsWithoutGST(
        productsCopy.filter((item) => item.gst == null || item.gst == -1)
      );
    }
  };
        ...
        <div className="row">
          <div className="col col-report row-border">
            <div className="row row-report ">
              <div className="col">
                <h4 className="report-title">Transactions</h4>
              </div>
              <div className="col">
                Search By Name :
                <input
                  type="text"
                  value={searchByPstName}
                  onChange={(e) => SearchByPstName(e)}
                ></input>
              </div>
              <div className="col">
                Search By Mobile :
                <input
                  type="text"
                  value={searchByPstMobile}
                  onChange={(e) => SearchByPstMobile(e)}
                ></input>
              </div>
            </div>
                  ...
                  <div className="row product-name-row" key={doc.id}>
                    ...
                    <div className="col-8">
                      {doc.items.map((b, i) => {
                        return (
                          <div className="row">
                            <div className="col-6">{b.name}</div>
                            <div className="col-3">{b.price}</div>
                            <div className="col-2">{b.quantity}</div>
                            {doc.editMode && (
                              <div className="col-1">
                                <button
                                  onClick={() =>
                                    deleteTransaction(doc, b, index)
                                  }
                                >
                                  X
                                </button>
                    ...
                    <div className="col-2">
                      ...
                      {Number(doc.discount)}
                      <div className="row">{doc.customerName}</div>
                      <div className="row">{doc.mobile}</div>
                    </div>

                    <div className="col-1">{doc.saleDate}</div>
                  </div>
                );
              })}
          </div>
        ...
        <div className="row">
          <div className="col-12 col-report row-border col-md-6 col-lg-6 col-xl-6">
                    ...
                    <div className="col-1">
                      <div className="row">
                        <div className="col unset-p-m">
                          <img
                            onClick={() => generateInvoice(doc, index)}
                            className="print-icon"
                            alt="edit"
                            src="/ebill/assets/digital.png"
                          />
                        </div>
                        <div className="col unset-p-m">
                          <img
                            onClick={() => generateInvoicePrint(doc, index)}
                            className="print-icon"
                            alt="edit"
                            src="/ebill/assets/print.png"
                          />
                        </div>
                        <div className="col unset-p-m">
                          <img
                            onClick={() => editSales(doc, index)}
                            className="print-icon"
                            alt="edit"
                            src="/ebill/assets/edit.png"
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div className="row row-h">
              ...
              <div className="col-2">{transactioByDateTotalPrice}</div>
              <div className="col-1"></div>
            </div>
          </div>
                      ...
                      return (
                        <div className="row border-b">
                          <div className="col">
                            ...
                            <div className="row">
                              <div className="col">Till Date Amount</div>
                              <div className="col trans-till-date">
                                <strong>{b.tranTillDate}</strong>
                              </div>
                            </div>
                          </div>

                          <div className="col-1">
                            <div className="row border-b">{round2(b.gst5)}</div>
                            <div className="row">{round2(b.gst5Value)}</div>
                          </div>
                          <div className="col-1">
                            <div className="row border-b">
                              {round2(b.gst12)}
                            </div>
                            <div className="row">{round2(b.gst12Value)}</div>
                          </div>
                          <div className="col-1">
                            <div className="row border-b">
                              {round2(b.gst18)}
                            </div>
                            <div className="row">{round2(b.gst18Value)}</div>
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="row row-h">
                ...
                <div className="col-5">
                  ...
                  <div className="row">
                    ...
                    <div className="col-4">Expiry</div>
                  </div>
                </div>
                <div className="col-1">Action</div>
              </div>
              ...
              {duplicateProducts &&
                duplicateProducts.map((doc, index) => {
                  return (
                    doc.duplicate && (
                      <div className="row product-name-row" key={doc.id}>
                        <div className="col-6">{doc.name}</div>
                        <div className="col-5">
                          {doc.batch.map((b, i) => {
                            return (
                              <div
                                className="row product-name-row"
                                key={i}
                                style={{
                                  backgroundColor:
                                    b.expiry === true ? b.colorCode : "white",
                                }}
                              >
                                <div className="col-4">{b.price}</div>
                        ...
                        <div className="col-1">
                          <button onClick={() => removeHandler(doc.id)}>
                            Remove
                          </button>
                        </div>
                      </div>
            ...
            <div className="row row-report">
              <div className="row ">
                ...
                <div className="col">
                  ...
                  <select
                    className="report-ddl"
                    onChange={(e) => {
                      SetByMonth(e.target.value);
                      filterItemsByMonthAndQuantity(e.target.value, byQuantity);
                    }}
                  >
                ...
                <div className="col">
                  ...
                  <select
                    className="report-ddl"
                    onChange={(e) => {
                      SetByQantity(e.target.value);
                      filterItemsByMonthAndQuantity(byMonth, e.target.value);
                    }}
                  >
                    <option value="1">1</option>
                    <option value="2">2</option>
              ...
              <div className="row row-h">
                ...
                <div className="col-6">Name</div>
                <div className="col-2">Sale Date</div>
                <div className="col-2">Total Qantity</div>
              </div>
                ...
                transactionsByItemsDisplay.map((b, i) => {
                  return (
                    <div className="row">
                      ...
                      <div className="col-6">{b.name}</div>
                      <div className="col-2">{b.saleDate}</div>
                      <div className="col-2">{b.totalQunatity}</div>
                    </div>
                  );
                })}
            </div>
          ...
          <div className="col col-report row-border">
            <div className="col col-report row-border">
              <div className="row row-report">
                <div className="row ">
                  <div className="col-4">
                    <h4 className="report-title">Tax updation</h4>
                  </div>
                  <div className="col-4">
                    Show All
                    <input
                      ...
                      name="showAll"
                      value={ShowAllTax}
                      onClick={(e) => ShowAllTaxInfo(e.target.value)}
                    />
                  </div>
                </div>
                <div className="row row-h">
                ...
                {productsWithoutGST &&
                  productsWithoutGST.map((b, i) => {
                    return (
                      <div className="row border-b">
                        <div className="col-2">{i + 1}</div>
                        <div className="col-8">{b.name}</div>
                        <div className="col-2">
                          <select
                            className="report-ddl"
                            value={b.gst}
                            onChange={(e) => {
                              seGst(e.target.value, b.id);
                            }}
                          >
                            <option
                              value="-1"
                              selected={b.gst === -1 || b.gst == null}
                            >
                              --Select--
                            </option>
                            <option value="0" selected={b.gst === 0}>
                              0
                            </option>
                            <option value="5" selected={b.gst === 5}>
                              5
                            </option>
                            <option value="12" selected={b.gst === 12}>
                              12
                            </option>
                            <option value="18" selected={b.gst === 18}>
                              18
                            </option>
                          </select>
                        </div>
                  ...
                  {mobileNumbers.map((b, i) => {
                    return (
                      <div className="row " key={i}>
                        <div className="col-4">{b.replace(/\D/g, "")}</div>
                      </div>
                    );
                  })}
                </Accordion.Body>
