...
function CashAccount() {
  ...
  const notify = () =>
    toast.success("Closing Balance Saved Successfully !", {
      position: "top-right",
      autoClose: 5000,
  ...
  useEffect(() => {
    try {
      ...
      let transactionDataByLocal = JSON.parse(
        localStorage.getItem("drkotianTransactionDataByMonth")
      );
      if (transactionDataByLocal) {
        let tempData = transactionDataByLocal.filter(
          (item) => item.id == getYearAndMonth()
        )[0];
        if (tempData) {
          tempData = tempData.transactions.filter(
            (ele) => ele.saleDate == getTodaysDate()
          );
      ...
      const unsub = onSnapshot(doc(db, "cash", id), (doc) => {
        ...
        if (!data) {
          ...
          if (data.dates.filter((item) => item.date == todayDate).length > 0) {
            let dailyData = data.dates.filter(
            ...
            let tempdailCashDenomination = Object.assign(
              {},
  ...
  useEffect(() => {
    let before =
      Number(dailyTotal) + Number(totalSales) - Number(cashAdjustment);
  ...
  useEffect(() => {
    SetDailyTotal(
      dailCashDenomination.reduce(
        (n, { opeiningTotal }) => n + opeiningTotal,
        0
      )
    );
    SetDailyTotalClosing(
      dailCashDenomination.reduce((n, { ClosingTotal }) => n + ClosingTotal, 0)
    );
  }, [dailCashDenomination]);
    ...
    SetDailCashDenomination(
      dailCashDenomination.map((item) =>
        item.id === index
          ? {
              ...item,
              openingCount: Number(count),
              opeiningTotal: Number(count) * Number(item.value),
            }
          : item
      )
    );
    ...
    SetDailCashDenomination(
      dailCashDenomination.map((item) =>
        item.id === index
          ? {
              ...item,
              closingCount: Number(count),
              ClosingTotal: Number(count) * Number(item.value),
            }
          : item
      )
    );
  ...
  const updateCashReport = async (dataToUpdate) => {
    ...
    if (docSnap.data()) {
      ...
      let todaysData = tempData.dates.filter(
        (item) => item.date == getTodayDate()
      )[0];
      if (todaysData) {
        tempData.dates.filter((item) => item.date == getTodayDate())[0].value =
          dataToUpdate;
        ...
        <div className="row">
          <div className="col-7 border-r border-t border-l">
            <div className="row row-h">
              <div className="col border-r">Denomination</div>
              <div className="col">Opening Count</div>
              <div className="col">Closing Count</div>
            </div>
            {dailCashDenomination.map((item, i) => {
              return (
                <div className="row border-b" key={i}>
                  ...
                  <div className="col">{item.openingCount}</div>
                  <div className="col">
                    <input
                      type="text"
                      value={item.closingCount}
                      onChange={(e) =>
                        updateDailyClosingSetData(i, e.target.value)
                      }
                    />
                  </div>
                </div>
          ...
          <div className="col-5">
            ...
            <div className="row">
              ...
              <div className="col">{dailyTotal} /-</div>
            </div>
            <div className="row">
              <div className="col">
                <strong>Sales Today</strong>
              </div>
              <div className="col">{totalSales} /-</div>
            </div>
            <div className="row">
              <div className="col">
                <strong>Closing Cash</strong>
              </div>
              <div className="col">{dailyTotalClosing} /-</div>
            </div>
            <div className="row">
              <div className="col">
            ...
            <div className="row">
              <div className="col">
                <strong>Difference Balance</strong>
              </div>
              <div className="col">
                <strong>
                  {Number(dailyTotal) +
                    Number(totalSales) -
                    Number(dailyTotalClosing) -
                    Number(onlinePayment) +
                    Number(cashAdjustment)}
                </strong>
              </div>
            </div>
        ...
        <Modal.Body>
          <>
            <div className="row">
              <div className="col-7 border-r border-t border-l">
                <div className="row row-h">
                  <div className="col-6 border-r">Denomination</div>
                    ...
                    <div className="row border-b" key={i}>
                      ...
                      <div className="col-6">
                        <input
                          type="text"
                          onChange={(e) =>
                            updateDailySetData(i, e.target.value)
                          }
                        />
                      </div>
                    </div>
